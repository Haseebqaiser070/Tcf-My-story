...
import { db } from "../../firebaseConfig"; // Import images statically
    ...
    const fetchSets = async () => {
      try {
        const querySnapshot = await getDocs(
          collection(db, "Quiz", "Written Expression", "Questions"),
        );
        const setsArray = [];
                      ...
                      >
                        ...
                        <View style={ExamStyle.proContainer}>
                          <ProgressSet
                            ...
                            } // Check if set.questions exists
                          />
                          <View style={ExamStyle.proTextContainer}>
                            <Text style={ExamStyle.cardTextDetail}>
                              {set.questions
                                ? `${set.questions.length}/${set.questions.length}`
                                : "0/0"}{" "}
                              {/* Display 0/0 if set.questions is undefined */}
                            </Text>
                          </View>
                        </View>
                      </TouchableOpacity>
