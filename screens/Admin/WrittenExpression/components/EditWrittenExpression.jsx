...
const EditWrittenExpression = () => {
    ...
    useEffect(() => {
        const fetchSetDetails = async () => {
            if (setId) {
                // Fetch the existing set details
                const setDocRef = doc(db, 'Quiz', 'Written Expression', 'Questions', setId);
    ...
    const saveMCQ = async () => {
        ...
        try {
            ...
            // Update the set number in Firestore if it's a new set
            if (!setId) {
                const setNumberDocRef = doc(db, 'Quiz', 'Written Expression', 'Questions', 'currentSetNumber');
                await setDoc(setNumberDocRef, { currentSet: setNumber + 1 });
            }

            Alert.alert('Success', `Question updated successfully in Set ${setNumber}!`);
            ...
            <ScrollView>
                <View style={examModuleStyle.subContainer}>
                    <View style={ssStyle.actionBar}>
                        <TouchableOpacity style={ssStyle.backButton} onPress={() => Navigator.navigate("WrittenExpressionSets")}>
