...
const WrittenExpressionSets = () => {
    ...
    useEffect(() => {
        const fetchSets = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, 'Quiz', 'Written Expression', 'Questions'));
                const setsArray = [];
    ...
    const navigateToSet = (setId) => {
        Navigator.navigate('EditWrittenExpression', { setId }); // Update the screen name as needed
    };
    ...
    return (
        <View style={examModuleStyle.mainConatiner}>
            <ScrollView>
                <View style={examModuleStyle.subContainer}>
                    <View style={ssStyle.actionBar}>
                        <TouchableOpacity style={ssStyle.backButton} onPress={() => {
                            Navigator.goBack();
                        }}>
                                            ...
                                            >
                                                ...
                                                <View style={ExamStyle.proContainer}>
                                                    <ProgressSet
                                                        ...
                                                        height={5}
                                                        color={"red"}
                                                        totalQuestions={set.questions ? set.questions.length : 0} // Check if set.questions exists
                                                    />
                                                    <View style={ExamStyle.proTextContainer}>
                                                        <Text style={ExamStyle.cardTextDetail}>
                                                            {set.questions ? `${set.questions.length}/${set.questions.length}` : '0/0'} {/* Display 0/0 if set.questions is undefined */}
                                                        </Text>
                                                    </View>
                                                </View>
                                            </TouchableOpacity>
            ...
            <TouchableOpacity style={examModuleStyle.addNewSetBtn} onPress={() => Navigator.navigate("WrittenExpressionAdmin")}>
...
export default WrittenExpressionSets;
