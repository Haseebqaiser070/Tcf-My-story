...
const EditListeningComprehension = () => {
    ...
    useEffect(() => {
        const fetchSetDetails = async () => {
            if (setId) {
                // Fetch the set details based on the dynamic parameter `setId`
                const setDocRef = doc(db, 'Quiz', 'Listening Comprehension', 'Mcqs', setId);
                const docSnap = await getDoc(setDocRef);

                if (docSnap.exists()) {
                    const data = docSnap.data();
                    setQuestions(data.questions || [{ audioUrl: '', options: ['', '', '', ''], correctOptionIndex: -1 }]);
                ...
                const setNumberDoc = doc(db, 'Quiz', 'Listening Comprehension', 'Mcqs', 'currentSetNumber');
    ...
    const saveMCQ = async () => {
        for (const question of questions) {
            ...
            if (question.options.some(option => !option) || question.correctOptionIndex === -1) {
                Alert.alert('Error', 'Please fill in all options for each question and mark the correct answer.');
                return;
            }
        }

        try {
            const setDocRef = doc(db, 'Quiz', 'Listening Comprehension', 'Mcqs', `set${setNumber}`);
            ...
            setQuestions([{ audioUrl: '', options: ['', '', '', ''], correctOptionIndex: -1 }]);
