...
import image6 from '../../img/ic_exam_set_purple.png'; // Use a different image if available
...
const OralExamSet = () => {
    ...
    useEffect(() => {
        const fetchSets = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, 'Quiz', 'Listening Comprehension', 'Mcqs'));
                const setsArray = [];
    ...
    const navigateToSet = (setId) => {
        Navigator.navigate('ListCompExam', {setId});
    };
                                        ...
                                        {row.map((set, index) => (
                                            ...
                                            >
                                                ...
                                                <View style={ExamStyle.cardDetail}>
                                                    ...
                                                    <View>
                                                        <Text style={ExamStyle.cardTextDetail}>{"Comprehension"}</Text>
                                                    </View>
                                                </View>
                                                <View style={ExamStyle.proContainer}>
                                                    ...
                                                    <View style={ExamStyle.proTextContainer}>
                                                        <Text style={ExamStyle.cardTextDetail}>
                                                            {set.questions ? `${set.questions.length}/${set.questions.length}` : '0/0'}
                                                        </Text>
                                                    </View>
                                                </View>
                                            </TouchableOpacity>
