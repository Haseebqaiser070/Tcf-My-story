...
const ListCompExam = ({ route }) => {
    ...
    const fetchQuestionsFromFirestore = async () => {
        ...
        try {
            const docRef = doc(db, 'Quiz', 'Listening Comprehension', 'Mcqs', setId);
    ...
    const handleNext = () => {
        if (currentQuestionIndex < questions.length - 1) {
            ...
            Navigator.navigate('ListCompResults', { data: 'LS', answers, questions, totalTime, setId });
        }
    };
    ...
    return (
        <>
            ...
            <Dialoge isOpen={isOpen} onClose={onClose} title={"Tip"} message={"Think out of the box to create it"} />
        </>
    );
};
